...
# data
gm_cleaned <- as.data.table(read_dta("MacroInsurance_Cleaned.dta"))[, ct := .N, .(pair)][npairs == 2]

gm_cleaned <- gm_cleaned[ct==2][(pair != "99999"),]
...
fwrite(final_table, "attrition-est-results-final-wo-cov-updated-2023.csv")
print(xtable(final_table[,-c("Estimates")], include.colnames=TRUE, booktabs=TRUE, type = "latex", digits = 3, caption = "Average Treatment Effect Estimates in Groh \\& McKenzie (2016)"),
      ...
      # add.to.row = comment, 
      hline.after = c(-1,0),
      file = "attrition-est-results-final-wo-cov-updated-2023.tex")
